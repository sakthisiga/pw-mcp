...
export class ProposalHelper {
  ...
  async createAndProcessProposal(leadModalLocator: string = '#lead-modal'): Promise<ProposalDetails> {
    CommonHelper.logger('INFO', '→ ENTER: createAndProcessProposal()');
    try {
      const { page } = this;
      ...
      CommonHelper.logger('INFO', '← EXIT: createAndProcessProposal() - Success');
      return {
        proposalNumber,
        services: selectedServices,
      };
    } catch (error) {
      CommonHelper.logger('ERROR', `← EXIT: createAndProcessProposal() - Failed: ${error}`);
      throw error;
    }
  }
  ...
  private async navigateToProposalsTab(dialog: any): Promise<void> {
    CommonHelper.logger('INFO', '→ ENTER: navigateToProposalsTab()');
    try {
  ...
  private async createProposalWithServices(): Promise<{
    ...
    CommonHelper.logger('INFO', '→ ENTER: createProposalWithServices()');
    try {
      // Select company
      ...
      CommonHelper.logger('INFO', '← EXIT: createProposalWithServices() - Success');
      return { selectedCompany, selectedServices };
    } catch (error) {
      CommonHelper.logger('ERROR', `← EXIT: createProposalWithServices() - Failed: ${error}`);
      throw error;
    }
  }
  ...
  private async selectCompany(): Promise<string> {
    CommonHelper.logger('INFO', '→ ENTER: selectCompany()');
    try {
      ...
      CommonHelper.logger('INFO', '← EXIT: selectCompany() - Success');
      return selectedCompany || '';
    } catch (error) {
      CommonHelper.logger('ERROR', `← EXIT: selectCompany() - Failed: ${error}`);
      throw error;
    }
  }
  ...
  private async triggerDropdownChange(): Promise<void> {
    CommonHelper.logger('INFO', '→ ENTER: triggerDropdownChange()');
    try {
      ...
      CommonHelper.logger('INFO', '← EXIT: triggerDropdownChange() - Success');
    } catch (error) {
      CommonHelper.logger('ERROR', `← EXIT: triggerDropdownChange() - Failed: ${error}`);
      throw error;
    }
  }
  ...
  private async selectService(): Promise<{ name: string; index: number }> {
    CommonHelper.logger('INFO', '→ ENTER: selectService()');
    try {
      ...
      CommonHelper.logger('INFO', '← EXIT: selectService() - Success');
      return { name: selectedService, index: randomServiceIndex };
    } catch (error) {
      CommonHelper.logger('ERROR', `← EXIT: selectService() - Failed: ${error}`);
      throw error;
    }
  }
  ...
  private async clickAddItemButton(): Promise<void> {
    CommonHelper.logger('INFO', '→ ENTER: clickAddItemButton()');
    try {
      const addItemBtn = this.page.locator('#btnAdditem');
      await expect(addItemBtn).toBeVisible();
      await addItemBtn.click();
      CommonHelper.logger('INFO', '← EXIT: clickAddItemButton() - Success');
    } catch (error) {
      CommonHelper.logger('ERROR', `← EXIT: clickAddItemButton() - Failed: ${error}`);
      throw error;
    }
  }
  ...
  private async selectSecondService(firstServiceIndex: number): Promise<string> {
    CommonHelper.logger('INFO', '→ ENTER: selectSecondService()');
    try {
      const serviceDropdown = this.page.locator('select').nth(1);
      const serviceOptions = await serviceDropdown.locator('option').allTextContents();
      const serviceIndices = serviceOptions.map((_, i) => i).filter(i => i > 0);
      ...
      CommonHelper.logger('INFO', '← EXIT: selectSecondService() - Success');
      return secondService;
    } catch (error) {
      CommonHelper.logger('ERROR', `← EXIT: selectSecondService() - Failed: ${error}`);
      throw error;
    }
  }
  ...
  private async saveProposal(): Promise<void> {
    CommonHelper.logger('INFO', '→ ENTER: saveProposal()');
    try {
  ...
  private async markProposalAsSent(): Promise<void> {
    CommonHelper.logger('INFO', '→ ENTER: markProposalAsSent()');
    try {
      ...
      CommonHelper.logger('INFO', '← EXIT: markProposalAsSent() - Success');
    } catch (error) {
      CommonHelper.logger('ERROR', `← EXIT: markProposalAsSent() - Failed: ${error}`);
      throw error;
    }
  }
  ...
  private async acceptAndDeclineServices(): Promise<void> {
    CommonHelper.logger('INFO', '→ ENTER: acceptAndDeclineServices()');
    try {
      ...
      CommonHelper.logger('ERROR', `← EXIT: acceptAndDeclineServices() - Failed: ${error}`);
      throw error;
    }
  }
  ...
  private async extractProposalNumber(): Promise<string> {
    CommonHelper.logger('INFO', '→ ENTER: extractProposalNumber()');
    try {
      ...
      CommonHelper.logger('INFO', '← EXIT: extractProposalNumber() - Success');
      return proposalNumber;
    } catch (error) {
      CommonHelper.logger('ERROR', `← EXIT: extractProposalNumber() - Failed: ${error}`);
      throw error;
    }
  }
}
