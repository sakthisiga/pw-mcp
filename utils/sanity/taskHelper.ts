...
export class TaskHelper {
  ...
  async createPaymentCollectionTask(): Promise<void> {
    CommonHelper.logger('INFO', '→ ENTER: createPaymentCollectionTask()');
    try {
      ...
      CommonHelper.logger('INFO', '← EXIT: createPaymentCollectionTask() - Success');
    } catch (error) {
      CommonHelper.logger('ERROR', `← EXIT: createPaymentCollectionTask() - Failed: ${error}`);
      throw error;
    }
  }

  private async clickNewTaskButton(): Promise<void> {
    CommonHelper.logger('INFO', '→ ENTER: clickNewTaskButton()');
    try {
    ...
    if (!clicked) {
      ...
      CommonHelper.logger('ERROR', '← EXIT: clickNewTaskButton() - Failed: No clickable candidate');
      throw new Error('Failed to click New Task (no clickable candidate)');
    }
    
    CommonHelper.logger('STEP', 'New Task button clicked');
    CommonHelper.logger('INFO', '← EXIT: clickNewTaskButton() - Success');
    } catch (error) {
      CommonHelper.logger('ERROR', `← EXIT: clickNewTaskButton() - Failed: ${error}`);
      throw error;
    }
  }

  private async tryElementHandleClick(candidates: Locator, candidateCount: number): Promise<boolean> {
    CommonHelper.logger('INFO', '→ ENTER: tryElementHandleClick()');
    for (let i = 0; i < candidateCount; i++) {
      ...
      if (box && box.width > 0 && box.height > 0) {
        await handle.click();
        CommonHelper.logger('INFO', '← EXIT: tryElementHandleClick() - Success (clicked)');
        return true;
      }
    }
    CommonHelper.logger('INFO', '← EXIT: tryElementHandleClick() - No clickable element found');
    return false;
  }
  ...
  private async tryAdvancedClickStrategies(candidates: Locator, candidateCount: number, actionsContainer: Locator): Promise<boolean> {
    CommonHelper.logger('INFO', '→ ENTER: tryAdvancedClickStrategies()');
    ...
    try {
      ...
      for (let i = 0; i < candidateCount; i++) {
        try {
          ...
          if (await maybeModal.count() && await maybeModal.isVisible()) {
            ...
            CommonHelper.logger('INFO', '← EXIT: tryAdvancedClickStrategies() - Success (evaluate click)');
            return true;
          }
        } catch (e) {
          // ignore and try next
      ...
      try {
        ...
        if (clickedViaDoc) {
          ...
          if (await maybeModal.count() && await maybeModal.isVisible()) {
            ...
            CommonHelper.logger('INFO', '← EXIT: tryAdvancedClickStrategies() - Success (document-level click)');
            return true;
          }
        }
      } catch (e) {
      ...
      try {
        ...
        if (aggressive && (aggressive as any).ok) {
          ...
          const maybeModal = this.page.locator('.modal:visible');
          if (await maybeModal.count() && await maybeModal.isVisible()) {
            CommonHelper.logger('INFO', '← EXIT: tryAdvancedClickStrategies() - Success (aggressive click)');
            return true;
          }
        } else {
      ...
      CommonHelper.logger('INFO', '← EXIT: tryAdvancedClickStrategies() - Failed (all strategies exhausted)');
      return false;
    } catch (error) {
      CommonHelper.logger('ERROR', `← EXIT: tryAdvancedClickStrategies() - Failed: ${error}`);
      throw error;
    }
  }

  private async waitForTaskModal(): Promise<void> {
    CommonHelper.logger('INFO', '→ ENTER: waitForTaskModal()');
    ...
    if (!modalAppeared) {
      ...
      CommonHelper.logger('ERROR', '← EXIT: waitForTaskModal() - Failed: Task modal not found');
    ...
    CommonHelper.logger('INFO', '← EXIT: waitForTaskModal() - Success');
  }

  private async fillTaskForm(): Promise<void> {
    CommonHelper.logger('INFO', '→ ENTER: fillTaskForm()');
    try {
      ...
      CommonHelper.logger('INFO', '← EXIT: fillTaskForm() - Success');
    } catch (error) {
      CommonHelper.logger('ERROR', `← EXIT: fillTaskForm() - Failed: ${error}`);
      throw error;
    }
  }

  private async saveTask(): Promise<void> {
    CommonHelper.logger('INFO', '→ ENTER: saveTask()');
    try {
      const taskModal = this.page.locator('.modal:visible');
      ...
      CommonHelper.logger('INFO', '← EXIT: saveTask() - Success');
    } catch (error) {
      CommonHelper.logger('ERROR', `← EXIT: saveTask() - Failed: ${error}`);
      throw error;
    }
  }

  private async handlePostSaveModal(): Promise<void> {
    CommonHelper.logger('INFO', '→ ENTER: handlePostSaveModal()');
    try {
      ...
      if (!postModalAppeared) {
        ...
        CommonHelper.logger('ERROR', '← EXIT: handlePostSaveModal() - Failed: Post-save modal not found');
      ...
      CommonHelper.logger('INFO', '← EXIT: handlePostSaveModal() - Success');
    } catch (error) {
      CommonHelper.logger('ERROR', `← EXIT: handlePostSaveModal() - Failed: ${error}`);
      throw error;
    }
  }
  ...
  private async closeModal(): Promise<void> {
    CommonHelper.logger('INFO', '→ ENTER: closeModal()');
    ...
    try {
      ...
      if (!modalExists) {
        ...
        CommonHelper.logger('INFO', '← EXIT: closeModal() - No modal to close');
        return;
      }
      ...
      CommonHelper.logger('INFO', '← EXIT: closeModal() - Success');
    } catch (error) {
      CommonHelper.logger('ERROR', `← EXIT: closeModal() - Failed: ${error}`);
      throw error;
    }
  }

  private async forceRemoveModal(): Promise<void> {
    CommonHelper.logger('INFO', '→ ENTER: forceRemoveModal()');
    try {
      ...
      CommonHelper.logger('INFO', '← EXIT: forceRemoveModal() - Success');
    } catch (error) {
      CommonHelper.logger('ERROR', `← EXIT: forceRemoveModal() - Failed: ${error}`);
      throw error;
    }
  }
  ...
  private async clearPageObstructions(): Promise<void> {
    CommonHelper.logger('INFO', '→ ENTER: clearPageObstructions()');
    ...
    try {
      ...
      await this.page.keyboard.press('Escape').catch(() => {});
      await this.page.waitForTimeout(500);
      CommonHelper.logger('INFO', '← EXIT: clearPageObstructions() - Success');
    } catch (err) {
      CommonHelper.logger('WARN', 'Error clearing page obstructions:', err);
      CommonHelper.logger('ERROR', `← EXIT: clearPageObstructions() - Failed: ${err}`);
      throw err;
    }
  }
  ...
  private async navigateToTasksTab(): Promise<void> {
    CommonHelper.logger('INFO', '→ ENTER: navigateToTasksTab()');
    ...
    try {
      ...
      if (currentUrl.includes('/admin') && !currentUrl.includes('/clients/') && !currentUrl.includes('/projects/')) {
        ...
        if (newUrl.includes('/admin') && !newUrl.includes('/clients/') && !newUrl.includes('/projects/')) {
          ...
          CommonHelper.logger('ERROR', `← EXIT: navigateToTasksTab() - Failed: Wrong page: ${newUrl}`);
      ...
      try {
        ...
        CommonHelper.logger('ERROR', `← EXIT: navigateToTasksTab() - Failed: Tasks tab not visible. Current URL: ${finalUrl}`);
      ...
      try {
        ...
        try {
          ...
          try {
            ...
            CommonHelper.logger('ERROR', '← EXIT: navigateToTasksTab() - Failed: Failed to click Tasks tab');
      ...
      try {
        ...
        CommonHelper.logger('ERROR', '← EXIT: navigateToTasksTab() - Failed: Tasks Summary heading not visible');
        throw new Error('Tasks Summary heading not visible after clicking tab');
      }
      
      CommonHelper.logger('INFO', '← EXIT: navigateToTasksTab() - Success');
    } catch (error) {
      CommonHelper.logger('ERROR', `← EXIT: navigateToTasksTab() - Failed: ${error}`);
      throw error;
    }
  }

  private async verifyTaskCreated(): Promise<void> {
    CommonHelper.logger('INFO', '→ ENTER: verifyTaskCreated()');
    try {
      ...
      CommonHelper.logger('INFO', '← EXIT: verifyTaskCreated() - Success');
    } catch (error) {
      CommonHelper.logger('ERROR', `← EXIT: verifyTaskCreated() - Failed: ${error}`);
      throw error;
    }
  }

  private async findPaymentCollectionTask(): Promise<boolean> {
    CommonHelper.logger('INFO', '→ ENTER: findPaymentCollectionTask()');
    try {
      ...
      for (let i = 0; i < 10; i++) {
        if (await taskRow.count() && await taskRow.isVisible()) {
          ...
          CommonHelper.logger('INFO', '← EXIT: findPaymentCollectionTask() - Found');
          return true;
        }
      ...
      CommonHelper.logger('INFO', '← EXIT: findPaymentCollectionTask() - Not found');
      return false;
    } catch (error) {
      CommonHelper.logger('ERROR', `← EXIT: findPaymentCollectionTask() - Failed: ${error}`);
      throw error;
    }
  }

  private async retryTaskCreation(): Promise<void> {
    CommonHelper.logger('INFO', '→ ENTER: retryTaskCreation()');
    try {
      ...
      for (let attempt = 0; attempt < 3; attempt++) {
        if (candidateCount > 0) {
          ...
          const paymentTaskFound = await this.findPaymentCollectionTask();
          if (paymentTaskFound) {
            CommonHelper.logger('INFO', '← EXIT: retryTaskCreation() - Success');
            return;
          }
        }
        await this.page.waitForTimeout(1000);
      ...
      CommonHelper.logger('ERROR', '← EXIT: retryTaskCreation() - Failed: Task not found after retries');
      throw new Error('Payment Collection task not found after creation and retry. Screenshot and HTML saved for debugging.');
    } catch (error) {
      CommonHelper.logger('ERROR', `← EXIT: retryTaskCreation() - Failed: ${error}`);
      throw error;
    }
  }
  ...
  private async captureTaskDetails(): Promise<void> {
    CommonHelper.logger('INFO', '→ ENTER: captureTaskDetails()');
    ...
    try {
      ...
      CommonHelper.logger('INFO', '← EXIT: captureTaskDetails() - Success');
    } catch (error) {
      CommonHelper.logger('WARN', `Failed to capture task details: ${error}`);
      CommonHelper.logger('ERROR', `← EXIT: captureTaskDetails() - Failed: ${error}`);
